...
export const ProgressView: React.FC<ProgressViewProps> = ({
  ...
  const badges = [
    { icon: '🎯', title: 'Titration Master', desc: 'Endpoint accuracy achieved within optimal range', unlocked: stats.avgScore >= 85 },
    { icon: '⚖️', title: 'Mass Preserver', desc: 'Mass stoichiometry verification compliant', unlocked: stats.totalCompleted >= 2 },
    { icon: '🥽', title: 'Safety Champion', desc: 'Zero severe laboratory safety penalty infractions', unlocked: stats.totalCompleted >= 3 },
    { icon: '⚡', title: 'Electro Chemist', desc: 'Completed conductance and ionic analysis practicals', unlocked: stats.totalCompleted >= 4 },
  ];
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
          <div>
            <h2 style={{ fontSize: '1.6rem', fontWeight: 800, fontFamily: 'var(--font-heading)', color: 'var(--text-primary)', margin: 0 }}>
              Laboratory Performance & Unified History
            </h2>
            <p style={{ fontSize: '0.84rem', color: 'var(--text-muted)', margin: '4px 0 0' }}>
              Comprehensive activity log tracking both your private classroom assessments and open practice sessions.
            </p>
          </div>
          {user && (
            <div
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14, marginBottom: 24 }}>
        {[
          ...
          },
          {
            ...
            sub: stats.avgScore >= 80 ? 'Class Honors Tier' : 'Active Progress',
            color: getScoreColor(stats.avgScore),
          },
          {
            label: 'Time in Laboratory',
            value: `${stats.totalTimeMinutes} mins`,
            sub: 'Hands-on interactive apparatus time',
            color: '#8b5cf6',
          },
          {
            label: 'Distinct Practicals',
            value: `${stats.distinctExperimentsCount}`,
            sub: 'Syllabus coverage index',
            color: '#f59e0b',
          },
        ].map((m, i) => (
          <div
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
            <h3 style={{ fontSize: '1.05rem', fontWeight: 700, color: 'var(--text-primary)', margin: 0 }}>
              Performance Records ({filteredHistory.length})
            </h3>
            <span style={{ fontSize: '0.72rem', color: 'var(--text-muted)' }}>
              Detailed breakdown of scores, attempts, and mistakes
            </span>
          </div>
          ...
          {filteredHistory.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '40px 20px', color: 'var(--text-muted)' }}>
              ...
              <p style={{ margin: 0, fontSize: '0.88rem' }}>No experiment records found under this filter.</p>
              <button
                onClick={() => onBackToHome()}
                style={{
                  all: 'unset',
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {filteredHistory.map((item) => {
                ...
                const gradeText = getGrade(item.score);
                    ...
                    <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
                      <div style={{ flex: 1, minWidth: 260 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                          {item.type === 'private_lab' ? (
                            ...
                            >
                              <span>🏫</span>
                              <span>Private Lab: {item.labCode || 'Exam'}</span>
                            </span>
                          ) : (
                            <span
                              style={{
                        ...
                        <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)', marginTop: 4 }}>
                          ...
                          {' '}• Time: <strong>{formatSeconds(item.timeSpentSeconds)}</strong>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                          ...
                          >
                            {isExpanded ? 'Hide Details ▲' : 'Inspect Mistakes ▼'}
                          </button>
                        </div>
                      </div>
                    </div>
                      ...
                      >
                        ...
                        <div>
                          <span style={{ fontWeight: 800, color: 'var(--text-primary)', display: 'block', marginBottom: 4 }}>
                            Logged Safety & Technique Observations:
                          </span>
                          {item.mistakes && item.mistakes.length > 0 ? (
                            ...
                            <span style={{ color: '#10b981', fontWeight: 600 }}>
                              ✅ Clean run! Zero procedural or calculation mistakes flagged by engine.
                            </span>
                          )}
                        </div>
